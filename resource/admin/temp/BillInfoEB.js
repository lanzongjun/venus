...
$(function () {
    //预导入CSV文件
    $('#bie_btn_do_input').bind('click', function () {
        $("#bie_form_input").form("submit", {
            type: 'post',
            url: '../'+__s_c_name+'/uploadInfo',
            onSubmit: function () {
                $('#bie_win_input').window('close');
                ajaxLoading();
            },
...
function appendData() {
    ...
    $.messager.confirm('确认', '预导入的数据将追加于正式数据之后，是否确认追加导入?', function (r) {
        if (r) {
            var s_tbn = $('#hid_tbn').val();
            //异步请求
            $.ajax({
