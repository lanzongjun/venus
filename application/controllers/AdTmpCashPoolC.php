 ...
 * @author Vincent
 */
class AdTmpCashPoolC extends CI_Controller {
    var $_s_view = 'TmpCashPool';
    var $_s_model = 'AdTmpCashPoolM';
    
    /**
     * 显示信息
     */
    function index() {
        $data['c_name'] = 'AdTmpCashPoolC';
    ...
    function getList() {
        ...
        $o_result = $this->AdTmpCashPoolM->_getList($i_page,$i_rows);
    ...
    function loadPreview() {
        $s_tbn = $_GET['tbn'];
        $this->load->model($this->_s_model);
        $o_result = $this->AdTmpCashPoolM->_loadTempTable($s_tbn);
        echo json_encode($o_result);
    }
    ...
    function appendData() {        
        $s_tbn = $_POST['tbn'];
        $this->load->model($this->_s_model);
        $b_result = $this->AdTmpCashPoolM->doAppendSQL($s_tbn);
    ...
    function coverData() {
        $s_tbn = $_POST['tbn'];
        $this->load->model($this->_s_model);
        $b_result = $this->AdTmpCashPoolM->doCoverSQL($s_tbn);
        $o_result = array('state'=>$b_result);
        echo json_encode($o_result);
    }
}
