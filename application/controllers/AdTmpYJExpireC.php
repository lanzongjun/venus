 ...
 * @author Vincent
 */
class AdTmpYJExpireC extends CI_Controller {
    var $_s_view = 'TmpYJExpire';
    var $_s_model = 'AdTmpYJExpireM';
    
    /**
     * 显示信息
     */
    function index() {
        $data['c_name'] = 'AdTmpYJExpireC';
    ...
    function getList() {
        ...
        $o_result = $this->AdTmpYJExpireM->_getList($i_page,$i_rows,$_GET);
        echo json_encode($o_result);
    }

    function getShopList() {        
        $this->load->model($this->_s_model);
        $o_result = $this->AdTmpYJExpireM->getShopList();        
        echo json_encode($o_result);
    }
    ...
    function uploadInfo() {
        ...
        if (!$result) {
            ...
            //获得临时表名
            $s_table_name_temp = $this->AdTmpYJExpireM->_getTempTableName();
            //导入临时表
            $i_rows = $this->AdTmpYJExpireM->inputCSV($s_table_name_temp, $o_result);
    ...
    function loadPreview() {
        $s_tbn = $_GET['tbn'];
        $this->load->model($this->_s_model);
        $o_result = $this->AdTmpYJExpireM->_loadTempTable($s_tbn);
        echo json_encode($o_result);
    }
    ...
    function coverData() {
        $s_tbn = $_POST['tbn'];
        $this->load->model($this->_s_model);
        $b_result = $this->AdTmpYJExpireM->doCoverSQL($s_tbn);
