 ...
 * @author Vincent
 */
class AdTmpOrderEBC extends CI_Controller {
    var $_s_view = 'TmpOrderEB';
    var $_s_model = 'AdTmpOrderEBM';
    
    /**
     * 显示信息
     */
    function index() {
        $data['c_name'] = 'AdTmpOrderEBC';
    ...
    function getList() {
        ...
        $o_result = $this->AdTmpOrderEBM->_getList($i_page,$i_rows);
    ...
    function uploadInfo() {
        ...
        if (!$result) {
            ...
            //获得临时表名
            $s_table_name_temp = $this->AdTmpOrderEBM->_getTempTableName();
            //导入临时表
            $i_rows = $this->AdTmpOrderEBM->inputCSV($s_table_name_temp, $o_result);
    ...
    function loadPreview() {
        $s_tbn = $_GET['tbn'];
        $this->load->model($this->_s_model);
        $o_result = $this->AdTmpOrderEBM->_loadTempTable($s_tbn);
        echo json_encode($o_result);
    }
    ...
    function appendData() {        
        $s_tbn = $_POST['tbn'];
        $this->load->model($this->_s_model);
        $b_result = $this->AdTmpOrderEBM->doAppendSQL($s_tbn);
